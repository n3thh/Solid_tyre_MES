...
class AdminUploaderApp:
    ...
    def setup_spec_tab(self):
        ...
        # 2. Bead Master
        tk.Label(f, text="B. Bead Master (Size Rules)", font=("Segoe UI", 11, "bold"), bg="white", fg="#E67E22").pack(pady=5)
    ...
    def download_sample_bead(self):
        ...
        try:
            with open(save_path, 'w', newline='') as f:
                writer = csv.writer(f)
                # NEW STRUCTURE
                writer.writerow(["TYRE_SIZE", "MOULD_ID", "BEAD_SIZE", "BEAD_COUNT"])
                writer.writerows([
                    ["18x7-8", "M-100", "8 Inch", 2],
                    ["23x9-10", "M-200", "10 Inch", 2]
                ])
            messagebox.showinfo("Success", "Sample Saved!")
        except Exception as e: messagebox.showerror("Error", str(e))
    ...
    def upload_plan(self):
        ...
        try:
            ...
            for _, r in df.iterrows():
                q = """
                INSERT INTO production_plan (press_id, daylight, tyre_size, brand, pattern, quality, mould_id_marks, type, tyre_weight, core_size)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (press_id, daylight) DO UPDATE SET tyre_weight=EXCLUDED.tyre_weight
                """
                ...
                params = (
                    ...
                    str(r.get('MOULD_ID', '')), str(r.get('TYPE', '')), wt, str(r.get('CORE_SIZE', ''))
                )
    ...
    def upload_beads(self):
        # Format: TYRE_SIZE, MOULD_ID, BEAD_SIZE, BEAD_COUNT
        path = filedialog.askopenfilename()
        if not path: return
        try:
            df = pd.read_csv(path)
            ...
            count = 0
            for _, r in df.iterrows():
                # We ignore Mould ID in logic for now, using Tyre Size as key
