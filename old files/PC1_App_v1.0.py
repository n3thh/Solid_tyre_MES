...
# CONFIGURATION
# =================================================
# Linux Printer Path (Standard for USB printers)
# Try /dev/usb/lp0 first. If it fails, check /dev/usb/lp1
...
class PC1SmartApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PC1 | Smart Green Tyre Builder (Linux Edition)")
    ...
    def build_tab2(self):
        ...
        e_scan.bind("<Return>", self.lookup_partial) 
    ...
    def reprint_selected(self):
        ...
        self.print_label(item['values'][0], item['values'][2], item['values'][3])

    def reprint_last(self):
        res = DBManager.fetch_data("SELECT b_id, tyre_size, status FROM pc1_building ORDER BY created_at DESC LIMIT 1")
        if res: self.print_label(res[0][0], res[0][1], res[0][2])
    ...
    def generate_report(self):
        ...
        if not res: return messagebox.showinfo("Empty", "No data found for this selection.")
    ...
    def load_specs(self, grade, size, p_type, mould_id):
        ...
        if not res_bead:
            query_bead_gen = "SELECT bead_size, bead_count, weight_per_bead FROM bead_master WHERE UPPER(tyre_size)=%s AND (mould_id='' OR mould_id IS NULL)"
            res_bead = DBManager.fetch_data(query_bead_gen, (clean_size,))
            
        if not res_bead:
            query_bead_fuzzy = "SELECT bead_size, bead_count, weight_per_bead FROM bead_master WHERE UPPER(tyre_size)=%s LIMIT 1"
            res_bead = DBManager.fetch_data(query_bead_fuzzy, (clean_size,))

        bead_wt_unit = 0.0
        bead_count = 0
        if res_bead:
            b = res_bead[0]
            self.disp_bead.set(f"{b[0]} x {b[1]}")
            bead_count = int(b[1] or 0)
            bead_wt_unit = float(b[2] or 0.0)
        else:
            self.disp_bead.set("⚠️ Bead Spec Missing")
        ...
        if res_grade:
            ...
            core_pct = float(r[0]) if r[0] else 0.0
            mid_pct = float(r[1]) if r[1] else 0.0
            ct_pct = float(r[2]) if r[2] else 0.0
            tread_pct = float(r[3]) if r[3] else 0.0
            
            total_bead_deduction = bead_count * bead_wt_unit
            net_rubber = self.target_weight - total_bead_deduction
            
            if net_rubber > 0:
                self.disp_core_tgt.set(f"{net_rubber*(core_pct/100.0):.2f} kg ({core_pct}%)")
                self.disp_mid_tgt.set(f"{net_rubber*(mid_pct/100.0):.2f} kg ({mid_pct}%)")
                self.disp_ct_tgt.set(f"{net_rubber*(ct_pct/100.0):.2f} kg ({ct_pct}%)")
                self.disp_tread_tgt.set(f"{net_rubber*(tread_pct/100.0):.2f} kg ({tread_pct}%)")
            else: self.disp_core_tgt.set("Wt Error")
        else: self.disp_core_tgt.set("Grade Spec Missing")

        if self.current_is_pob:
            self.row_core_tgt.pack_forget(); self.row_mid_tgt.pack_forget(); self.row_bead_tgt.pack_forget()
            self.row_core_input.pack_forget(); self.row_mid_input.pack_forget()
            self.frame_gum.pack(fill="x", pady=5) 
        else:
            self.row_core_tgt.pack(fill="x"); self.row_mid_tgt.pack(fill="x"); self.row_bead_tgt.pack(fill="x")
            self.row_core_input.pack(fill="x"); self.row_mid_input.pack(fill="x")
            self.frame_gum.pack_forget()
    ...
    def submit_build(self, mode):
        ...
        if not self.current_is_pob:
            if not self.get_list_values("list_core"): return messagebox.showerror("Error", "Core Batch Required!")
        else:
        ...
        if DBManager.execute_query(q, d):
            self.print_label(bid, self.var_size.get(), mode)
    ...
    def lookup_partial(self, event):
        bid = self.upd_bid.get().strip()
        if not bid: return
        res = DBManager.fetch_data("SELECT tyre_size, quality, status FROM pc1_building WHERE b_id=%s", (bid,))
        if res:
            if res[0][2] == "COMPLETED": self.lbl_partial_info.config(text=f"⚠️ {bid} is COMPLETED!", fg=C_ERROR)
            else: self.lbl_partial_info.config(text=f"✅ Found: {res[0][0]} | {res[0][1]}", fg=C_SUCCESS)
        else: self.lbl_partial_info.config(text="❌ Not Found", fg=C_ERROR)
    ...
    def create_kv(self, parent, label, var, bold=False):
        ...
        tk.Label(f, textvariable=var, anchor="w", bg=C_CARD, font=font, fg=C_ACCENT if bold else "black").pack(side="left")
        return f
    ...
    def create_multi_select(self, parent, label, var_sel, list_attr):
        ...
        setattr(self, f"combo_{list_attr}", cb) # <--- FIXED: This restores the missing attribute!
        
        # Use lambda with default args to capture current variables
        tk.Button(row, text="+", command=lambda l=list_attr, v=var_sel: self.add_item(l, v), bg=C_ACCENT, fg="white", width=3).pack(side="right")
        lb = tk.Listbox(f, height=2, bg="#FAFAFA"); lb.pack(fill="x")
        setattr(self, list_attr, lb)
        tk.Button(f, text="Clear", command=lambda: getattr(self, list_attr).delete(0, tk.END), font=("Segoe UI", 7)).pack(anchor="e")
        return f
    ...
    def load_operators(self):
        res = DBManager.fetch_data("SELECT full_name FROM users WHERE is_active=TRUE")
        if res: self.combo_op['values'] = [r[0] for r in res]
    def load_press_list(self):
        res = DBManager.fetch_data("SELECT DISTINCT press_id FROM production_plan")
        if res: self.combo_press['values'] = [r[0] for r in res]
    def on_press_change(self, event):
        res = DBManager.fetch_data("SELECT daylight FROM production_plan WHERE press_id=%s", (self.var_press.get(),))
        if res: self.combo_daylight['values'] = [r[0] for r in res]
    def load_materials(self):
        res = DBManager.fetch_data("SELECT batch_no FROM raw_material_qc WHERE material_type='GUM' AND status='APPROVED'")
        if res: self.combo_gum_widget['values'] = [r[0] for r in res]
        res = DBManager.fetch_data("SELECT batch_no FROM raw_material_qc WHERE material_type='CORE' AND status='APPROVED'")
        if res: getattr(self, "combo_list_core")['values'] = [r[0] for r in res]
        res = DBManager.fetch_data("SELECT batch_no FROM raw_material_qc WHERE (material_type='MIDDLE_LAYER' OR material_type='MIDDLE LAYER') AND status='APPROVED'")
        if res: getattr(self, "combo_list_mid")['values'] = [r[0] for r in res]
    def on_tread_type_change(self, event):
        tt = self.sel_tread_type.get()
        res = DBManager.fetch_data("SELECT batch_no FROM raw_material_qc WHERE material_type=%s AND status='APPROVED'", (tt,))
        if res: getattr(self, "combo_list_tread")['values'] = [r[0] for r in res]
    def on_upd_tread_type_change(self, event):
        tt = self.upd_tread_type.get()
        res = DBManager.fetch_data("SELECT batch_no FROM raw_material_qc WHERE material_type=%s AND status='APPROVED'", (tt,))
        if res: getattr(self, "combo_list_upd_tread")['values'] = [r[0] for r in res]
    ...
    def print_label(self, bid, size, status):
        # TSPL COMMANDS for 58x40mm (Approx)
        # Adjust "SIZE 58 mm, 40 mm" if your label size differs
        tspl_data = f"""
SIZE 58 mm, 40 mm
GAP 2 mm, 0 mm
DIRECTION 1
CLS
TEXT 10,10,"3",0,1,1,"{bid}"
TEXT 10,40,"2",0,1,1,"{size}"
TEXT 10,70,"2",0,1,1,"{status}"
BARCODE 10,100,"128",80,1,0,2,2,"{bid}"
PRINT 1
"""
        try:
            if platform.system() == "Linux":
                with open(LINUX_PRINTER_PATH, "wb") as f:
                    f.write(tspl_data.encode())
                print(f"Sent to {LINUX_PRINTER_PATH}")
            else:
                print("Windows Printing not implemented in this snippet")
        except Exception as e:
