...
class PC1SmartApp:
    ...
    def build_tab2(self):
        ...
        # 2. Listen for Tab Key (Common in scanners)
        e_scan.bind("<Tab>", self.lookup_partial) 
        # 3. Listen for "Auto-Type" (If scanner types fast, trigger at 8 chars)
        self.upd_bid.trace_add("write", self.check_scan_length)
    ...
    def check_scan_length(self, *args):
        ...
        if len(text) == 8 and text.startswith("B-"):
            print(f"🔹 Auto-detected Scan: {text}")
    ...
    def reprint_last(self):
        ...
        if res: 
            self.print_label(res[0][0], res[0][1], res[0][2], res[0][3])
        else: 
    ...
    def submit_build(self, mode):
        ...
        if DBManager.execute_query(q, d):
            # PRINTING CALL (With updated arguments)
    ...
    def print_label(self, bid, size, press, daylight):
        # EPL COMMANDS
        # q464 = Width 58mm
        # Q312,24 = Length 39mm (Reduced from 40mm to stop skipping), Gap 3mm
        # B... 2,5 = Narrow 2 dots, Wide 5 dots (Perfect 1:2.5 ratio for scanners)
        epl_data = f"""
N
q464
Q200,22
A47,30,0,3,1,1,N,"{size} | {press}-{daylight}"
B60,60,0,1,2,8,90,B,"{bid}"
P1
"""
        try:
